...
export default {
  ...
  async mounted() {
    await this.getRouteResourceDetail()
    await this.getRouteResourceOptions()
  },
  methods: {
    getRouteResourceDetail() {
      return routeResourceDetail({ id: this.ruleForm.id })
        .then((res) => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          routeResourceEdit(this.ruleForm)
            .then((res) => {
              this.$message({
                message: '保存成功',
                type: 'success',
